...
- (NSData *)gunzippedData:(NSData*)data
{
    if ([data length])
    {
        ...
        NSMutableData *mutable = [NSMutableData dataWithLength: [data length] * 1.5];
        if (inflateInit2(&stream, 47) == Z_OK)
        {
            int status = Z_OK;
            while (status == Z_OK)
            {
                if (stream.total_out >= [mutable length])
                {
                    mutable.length += [data length] * 0.5;
                }
                stream.next_out = [mutable mutableBytes] + stream.total_out;
                stream.avail_out = (uint)([mutable length] - stream.total_out);
            ...
            if (inflateEnd(&stream) == Z_OK)
            {
                ...
                {
                    mutable.length = stream.total_out;
                    return mutable;
                }
            }
        }
